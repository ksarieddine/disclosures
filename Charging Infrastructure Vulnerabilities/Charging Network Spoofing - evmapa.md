...
### Steps to Reproduce
1. In a controlled test environment, identify EVMAPA WebSocket and authentication endpoints used by charging stations.
2. Attempt a WebSocket handshake without valid credentials and verify whether backend access is still granted.
3. Perform bounded authentication-attempt testing to confirm whether lockout or rate-limiting controls are enforced.
4. Start parallel backend sessions using the same charging station ID and verify whether prior sessions are invalidated and expired correctly.
